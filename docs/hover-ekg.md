...
Medtech requires a native role item with positive rank, identified by its Medtech name, role name or native compendium source. Selecting a non-Medtech does not borrow the assigned character role. GMs also need a Medtech or the world override.

This feature is independent of the hover-DV toggle, weapon selection, combat state and Biomonitor implant requirements. It respects native token visibility and does not expose hidden tokens. The display disappears on hover exit, scene teardown, token deletion or browser blur. HP and role changes refresh it. Pan/zoom repositions the panel while the animation remains stable when the health state is unchanged.
